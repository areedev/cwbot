...
const handleMail = async (content, type, message, source, eventEmitter) => {
  ...
  if (message.title.toString() != '【クラウドワークス】新規会員登録を完了してください' ||
    message.from.address.toString() != 'no-reply@crowdworks.jp' ||
    message.to[0].name.toString() != `${mail.user.substring(0, mail.user.indexOf('@'))}+${mail.no}@${mail.user.substring(mail.user.indexOf('@') + 1)}`)
    return;
